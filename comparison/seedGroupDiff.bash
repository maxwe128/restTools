...
for sub in $(cat $group1SubList);do
	...
	if [[ ! -f ${outDir}/Zfiles/$prefix.$sub.maskConnData.Z.nii ]];then
		echo "3dmaskave -quiet -mrange $maskSelector $maskSelector -mask $seedMask $subData > tmp.$prefix.$sub.maskData.1D"
		...
		mv $prefix.$sub.maskConnData.Z.nii ${outDir}/Zfiles/
	else
		echo "values previously extracted for $sub"
	fi
	echo -e "\t$sub $outDir/Zfiles/$prefix.$sub.maskConnData.Z.nii \\" >> $outDir/$prefix.ttestCommand.bash	
done
...
if [[ $age == "T" ]] || [[ $sex == "T" ]] || [[ $motion == "T" ]];then
	...
	echo -e "\t-covariates $outDir/$prefix.covFile.txt" >> $outDir/$prefix.ttestCommand.bash
else
